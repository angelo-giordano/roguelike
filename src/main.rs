    ...
    pub fn equip(&mut self, messages: &mut Messages) {
        ...
        if let Some(ref mut equipment) = self.equipment {
            if !equipment.equipped {
                ...
                messages.add(
                    format!("Equipado {} na {}.", self.name, equipment.slot),
                    LIGHT_GREEN,
                );
            }
        } else {
    ...
    pub fn dequip(&mut self, messages: &mut Messages) {
        ...
        if let Some(ref mut equipment) = self.equipment {
            if equipment.equipped {
                ...
                messages.add(
                    format!("Desequipado {} da {}.", self.name, equipment.slot),
                    LIGHT_YELLOW,
                );
            }
        } else {
...
fn use_item(inventory_id: usize, tcod: &mut Tcod, game: &mut Game, objects: &mut [Object]) {
    ...
    if let Some(item) = game.inventory[inventory_id].item {
        ...
        match on_use(inventory_id, tcod, game, objects) {
            ...
            UseResult::Cancelled => {
                game.messages.add("Cancelado", WHITE);
            }
        }
    } else {
        game.messages.add(
...
fn cast_lightning(
    ...
    if let Some(monster_id) = monster_id {
        ...
        game.messages
            .add("Sem inimigos por perto.", RED);
        UseResult::Cancelled
    }
}
...
fn cast_fireball(
    ...
    game.messages.add(
        format!(
            "O fogo toma conta do local, queimando tudo dentro de {} blocos!",
            FIREBALL_RADIUS
        ),
        ORANGE,
    );
    ...
    for (id, obj) in objects.iter_mut().enumerate() {
        if obj.distance(x, y) <= FIREBALL_RADIUS as f32 && obj.fighter.is_some() {
            game.messages.add(
                format!(
                    "{} é queimado e perde {} pontos de vida.",
        ...
        if !is_blocked(x, y, map, objects) {
            let mut item = match item_choice.ind_sample(&mut rand::thread_rng()) {
                Item::Heal => {
                    ...
                    let mut object = Object::new(x, y, '!', "Canela de Velho", VIOLET, false);
                ...
                Item::Fireball => {
                    ...
                    let mut object =
                        Object::new(x, y, '#', "Pergaminho do Fogo", LIGHT_YELLOW, false);
                ...
                Item::Confuse => {
                    ...
                    let mut object =
                        Object::new(x, y, '#', "Pergaminho do Álcool", LIGHT_YELLOW, false);
                    object.item = Some(Item::Confuse);
                    object
                }
                Item::Sword => {
                    // create a sword
                    let mut object = Object::new(x, y, '/', "Espada Curta", SKY, false);
                ...
                Item::Shield => {
                    // create a shield
                    let mut object = Object::new(x, y, '[', "Escudo", DARKER_ORANGE, false);
...
fn render_all(tcod: &mut Tcod, game: &mut Game, objects: &[Object], fov_recompute: bool) {
    ...
    tcod.panel.print_ex(
        ...
        BackgroundFlag::None,
        TextAlignment::Left,
        format!("Andar da dungeon: {}", game.dungeon_level),
    );
            ...
            .map(|item| {
                ...
                match item.equipment {
                    Some(equipment) if equipment.equipped => {
                        format!("{} ({})", item.name, equipment.slot)
                    }
                    _ => item.name.clone(),
                }
            })
...
fn monster_death(monster: &mut Object, game: &mut Game) {
    ...
    monster.ai = None;
    monster.name = format!("restos de {}", monster.name);
}
...
fn new_game(tcod: &mut Tcod) -> (Game, Vec<Object>) {
    ...
    let mut dagger = Object::new(0, 0, '-', "Adaga", SKY, false);
    ...
    game.messages.add(
        "Bem Vindo, viajante! Prepare-se para entrar nas profundezas da masmorra.",
        RED,
    );
...
fn main_menu(tcod: &mut Tcod) {
    ...
    while !tcod.root.window_closed() {
        ...
        let choices = &["Novo Jogo", "Continuar", "Sair"];
